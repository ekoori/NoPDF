...
public static class CommandDocs
{
    ...
    public static readonly IReadOnlyList<CommandDoc> All = new[]
    {
        ...
        new CommandDoc("tabname", "File", "[name]",
            "Label the current tab (blank clears it) — renames the tab, never the file", "tabrename"),
        new CommandDoc("tabmove", "File", "<position|left|right|first|last>",
            "Move the current tab within the tab strip"),
